"""
Python conversion of the MATLAB script `geostat_electrons_spec_flux.m`
by Eran Ofek, available at:
  https://github.com/EranOfek/AstroPack/blob/main/matlab/astro/+ultrasat/geostat_electrons_spec_flux.m

This module provides a spectral model for electron flux in geostationary orbit.
"""
...
def geostat_electrons_spec_flux(source="AE9"):
    """
    Return a spectral model for electron flux in geostationary orbit.

    This model provides daily mean and quantile estimates of electron fluxes
    at multiple energy levels, based on the AE9 empirical model.

    Parameters
    ----------
    source : str, optional
        Name of the source model (default 'AE9').
    ...
    The returned columns include:
        - Energy (MeV)
        - DailyMin_MeanFlux, DailyMax_MeanFlux
        - DailyMin_95Flux, DailyMax_95Flux
        - DailyMax_50Flux, DailyMax_75Flux
        - DailyMin_50Flux, DailyMin_75Flux

    All fluxes are in units of counts(>E)/cm²/s.

    Examples
    --------
    >>> flux = geostat_electrons_spec_flux()
    >>> flux['data']['DailyMin_MeanFlux']
    """
    # Fmat1: Energy, DailyMin_Mean, DailyMax_Mean, DailyMin_95, DailyMax_95
    Fmat1 = np.array(
        [
    ...
    # Fmat2: Energy, DailyMax_MeanFlux , DailyMax_50, DailyMax_75,  DailyMax_95Flux
    # To be used : _, _,  DailyMax_50, DailyMax_75, _
    Fmat2 = np.array(
        [
    ...
    # # Fmat3 : Energy, DailyMin_MeanFlux, DailyMin_50Flux, DailyMin_75Flux, DailyMin_95Flux
    # to be use : _, _, DailyMin_50, DailyMin_75, _
    ...
    # Combine columns as in the original MATLAB code
    combined = np.column_stack(
        [
            Fmat1,
            Fmat2[:, 2:4],  # DailyMax 50% and 75%
            Fmat3[:, 2:4],  # DailyMin 50% and 75%
        ]
    )
    ...
    table = Table(combined, names=col_names)

    return {
        "source": source,
        "columns": col_names,
        "units": ["MeV"] + ["counts(>E)/cm²/s"] * (len(col_names) - 1),
        "data": table,
    }
